...
class DashboardController {
  ...
  static renderKPIs(summary) {
    ...
    setElem('kpi-total', summary.total_incidents ?? 0);
    setElem('kpi-open', summary.open_incidents ?? 0);
    setElem('kpi-critical', summary.critical_incidents ?? 0);
    setElem('kpi-resolved', summary.resolved_incidents ?? 0);
    setElem('kpi-today', summary.created_today ?? 0);

    const mttr = summary.avg_resolution_time_minutes;
    setElem('kpi-mttr', mttr ? `${mttr}m` : 'N/A');
  }
